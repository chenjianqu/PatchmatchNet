...
class MaxieyeMVSDataset(Dataset):
    def __init__(
        ...
        self.train_list = [data_root + '/' + dataset for dataset in train_datasets]
        self.train_lidar_list = [lidar_data_root + '/' + dataset for dataset in train_datasets]
    ...
    def __getitem__(self, idx):
        ...
        for i, image_idx in enumerate(view_ids):
            ...
            T_wc = poses[image_idx].A
            intrinsics_list.append(K_un)
        ...
        ref_img = images[0].transpose([1, 2, 0])
        depth_img = None

        timestamp = Path(ref_image_path).stem + ".png"
        depth_path = os.path.join(self.train_lidar_list[sce_id], "depths", timestamp)

        if self.regenerate_depth_image or (not os.path.exists(depth_path)):
            depth_img = np.zeros((H, W), dtype=np.float32)

            # 将所有的点云融合到参考视图
            _, ref_T_wl0 = self.ixes[sce_name][0][sce_id_ind]
            for i, image_idx in enumerate(view_ids):
                image_sce_id, image_sce_id_ind = self.data_idxs[image_idx]
                image_path, T_wli = self.ixes[sce_name][0][image_sce_id_ind]
                T_l0li = np.linalg.inv(ref_T_wl0).dot(T_wli)
                T_l0li = T_l0li.A
                ...
                #    cv2.circle(ref_img, (xy[0, i], xy[1, i]), 2, (255, 0, 255), -1)
